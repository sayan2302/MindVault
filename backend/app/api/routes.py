...
async def upload_document(file: UploadFile = File(...)) -> dict[str, object]:
    ...
    # Auto-index pipeline (V1 synchronous)
    try:
        docs = load_documents_from_file(destination)
        chunks = chunk_documents(docs)
        index_result = index_chunks(chunks)

        return {
            "message": "Upload successful and auto-indexed",
            "status": "indexed",
            "stored_filename": unique_name,
            "size_bytes": len(content),
            "storage_path": str(destination),
        ...
        # Keep uploaded file; report indexing failure clearly
        return {
            "message": "Upload successful, but indexing failed",
